...
# Stop josh if its running already
sudo service josh stop > /dev/null 2>&1 || true
...
if [ -d "/usr/lib/systemd/system" ]; then
    # systemd is present normally
    sudo bash -c 'cat sysconfigs/systemd.service > /usr/lib/systemd/system/josh.service'
else
    # systemd is present but in a different directory
    sudo bash -c 'cat sysconfigs/systemd.service > /lib/systemd/system/josh.service'
fi
...
VERSION=$(cat package.json | grep version | tr -d ", ")
